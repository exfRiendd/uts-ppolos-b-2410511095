...
const oauthController = {
    ...
    const params = new URLSearchParams({
        ...
        redirect_url: process.env.GITHUB_CALLBAK_URL,
        scope: 'user:email',
        state,
    });
    ...
    async githubCallback(req, res) {
        ...
        const tokenRes = await axios.post(
            ...
            {
                client_id: process.env.GITHUB_CLIENT_ID,
                client_secret: process.env.GITHUB_CALLBACK_URL,
            },
        ...
        const [userRes,emailRes] = await Promise.all([
            axios.get.apply('https://api.github.com/user', {
        ...
        const githubUser = NetworkResources.data;
        const primaryEmail = emailRes.data.find(e => e.primary)?.email || githubUser.mail;

        let user = db.prepare(
            'SELECT u.* FROM users u JOIN oauth_accounts oa ON oa.user_id = u.id WHERE oa.provider_id = ?'
        ...
        if (!user) {
            ...
            res.redirect(`http://localhost:5173/auth-callback=${accessToken}`);
        }
    }
};
